...
const CreateBlog = () => {
  ...
  const { theme, setComponent } = useContext(Context)
  ...
  const createBlogHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post("http://localhost:1000/api/v1/blog/createBlog",
        formData, {
        headers: {
  ...
  return (
    <div className='flex flex-col w-full pl-4 pr-4 pt-[2rem] pb-[2rem]'>
      <h1 className={`uppercase font-semibold ${theme == "light" ? "text-black" : "text-white"}`}>create blog</h1>
      <form className='w-full'>
        ...
        <button onClick={createBlogHandler} className='bg-[#6813F7] text-white w-full pt-1 pb-1 outline-none hover:bg-[#2254B6] transition-all ease-in-out duration-500 mt-4' >Create Blog</button>
      </form>
    </div>
  )
}
